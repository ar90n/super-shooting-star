...
import { URL } from 'url';
import { DUMMY_ACCOUNT } from '../models/account';
import S3Error from '../models/error';
import S3Event from '../models/event';
import S3Object from '../models/object';
import { TaggingConfiguration } from '../models/config';
import { capitalizeHeader, utf8BodyParser, xmlBodyParser } from '../utils';
 ...
 */
export const deleteMultipleObjects = async function deleteMultipleObjects(ctx) {
 ...
 */
export const deleteObject = async function deleteObject(ctx) {
  try {
 ...
 */
export const getObjectAcl = async function getObjectAcl(ctx) {
 ...
 */
export const getObjectTagging = async function getObjectTagging(ctx) {
  const key = ctx.params.key;
 ...
 */
export const postObject = async function postObject(ctx) {
    ...
    const object = await Promise.race([
      ...
      new Promise((resolve, reject) => {
        ...
        const fieldHandler = (fieldname, value) => {
          if (fieldname === 'file') {
            //  will identify parts as fields if they both lack a filename
 ...
 */
export const putObject = async function putObject(ctx) {
 ...
 * @returns {Promise<void>}
 */
export const putObjectTagging = async function putObject(ctx) {
  const key = ctx.params.key;
 ...
 */
export const completeMultipartUpload = async function completeMultipartUpload(
  ctx,
) {
  await xmlBodyParser(ctx);
  if (
 ...
 */
export const initiateMultipartUpload = async function initiateMultipartUpload(
  ctx,
) {
 ...
 */
export const uploadPart = async function uploadPart(ctx) {
  aws4SignatureBodyParser(ctx);
  try {
